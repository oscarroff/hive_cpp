...
// Forward declarations of fordJohnsonAlgo() dependencies
template <typename T>
...
// Find the max element size for merge insertion
// For this we need a minimum 3 of a given element size, representing b1, a1 and
// the element to be inserted b2
static size_t	maxElementSize( size_t dataSize ) {
...
// Algorithm for finding the comparison tree size
// This could be further optimised by ommitting one element from the tree if the
// previous insertion was placed to the far right, i.e. a-max in the main
// But this optimisation was considered out of scope for this implementation
static size_t	fetchTreeSize( size_t jN, size_t jC, size_t jP, size_t pS, size_t eS ) {
...
// When moving the upper and lower bounds of the search area of the comparison
// tree we need to increment and decrement by multiples of the element size
// thus this algorithm comes in handy
static size_t	roundToScale( size_t value, size_t scale ) {
